function ReportROIinformation_cn(dat,dat2,hdr,hdr2,outputs)
...
string1 = ['全ROI体素数目: ',num2str(nnz(dat2))];
almat = hdr.mat(1:3,1:3);
 vsize = [max(abs(almat(1,:))),max(abs(almat(2,:))),max(abs(almat(3,:)))];
vols = prod(vsize)*nnz(dat2);
volshow = round(vols);
string2 = ['全ROI体积(毫升): ',num2str(volshow/1000)];
string3 = ['ROI内最小/最大值: ',num2str(min(dat(indexs))),'/',num2str(max(dat(indexs)))];
% string4 = ['max value in ROI: ',num2str(max(dat(indexs)))];
string5 = ['ROI内平均值/方差: ',num2str(mean(dat(indexs))),'/',num2str(std(dat(indexs)))];
% string6 = ['std : ',num2str(std(dat(indexs)))];
string7 = ['ROI内中位数: ',num2str(median(dat(indexs)))];
datsort = sort(dat(indexs));
string8 = ['ROI内1/4位数: ',num2str(datsort(round(length(datsort)/4)))];
string9 = ['ROI内3/4位数: ',num2str(datsort(round(length(datsort)/4*3)))];
KUR = kurtosis(dat(indexs));
string10 = ['ROI内灰度分布峰度: ',num2str(KUR)];
SKE = skewness(dat(indexs));
string11 = ['ROI内灰度分布偏度: ',num2str(SKE)];
...
string0s = ['当前层: 横断位 ',num2str(ZDIMS),'(范围: 1-',num2str(dims(3)),')'];
if length(INDS)>0
    string1s = ['层内ROI体素数目: ',num2str(nnz(SELTEMPMAP2))];
    almat = hdr.mat(1:3,1:3);
    vsize = [max(abs(almat(1,:))),max(abs(almat(2,:))),max(abs(almat(3,:)))];
    vols = prod(vsize)*nnz(SELTEMPMAP2);
    volshow = round(vols);
    string2s = ['层内ROI体积（毫升）: ',num2str(volshow/1000)];
    string3s = ['层内ROI最小/最大值: ',num2str(min(SELTEMPMAP(INDS))),'/',num2str(max(SELTEMPMAP(INDS)))];
%     string4s = ['max value in ROI in slice: ',num2str(max(SELTEMPMAP(INDS)))];
    string5s = ['层内ROI平均值: ',num2str(mean(SELTEMPMAP(INDS)))];
    string6s = ['方差 : ',num2str(std(SELTEMPMAP(INDS)))];
    string7s = ['层内ROI中位数: ',num2str(median(SELTEMPMAP(INDS)))];
    datsorts = sort(SELTEMPMAP(INDS));
    string8s = ['层内ROI较小/大四分位数（25%/75%）: ',num2str(datsorts(round(length(datsorts)/4))),'/',num2str(datsorts(round(length(datsorts)/4*3)))];
%     string9s = ['Q3(75%) value in ROI in slice: ',num2str(datsorts(round(length(datsorts)/4*3)))];
    KUR = kurtosis(SELTEMPMAP(INDS));
    string10s = ['层内ROI灰度分布峰度: ',num2str(KUR)];
    SKE = skewness(SELTEMPMAP(INDS));
    string11s = ['层内ROI灰度分布偏度: ',num2str(SKE)];
else
    string1s = ['层内ROI体素数目: 0'];
    string2s = ['层内ROI体积（毫升）: 0'];
    string3s = ['层内ROI最小/最大值: NaN/NaN'];
%     string4s = ['max value in ROI in slice: NaN'];
    string5s = ['层内ROI平均值: NaN'];
    string6s = ['方差 : NaN'];
    string7s = ['层内ROI中位数: NaN'];
    string8s = ['层内ROI较小/大四分位数（25%/75%）: NaN/NaN'];
%     string9s = ['Q3(75%) value in ROI in slice: NaN'];
    string10s = ['层内ROI灰度分布峰度: NaN'];
    string11s = ['层内ROI灰度分布偏度: NaN'];
end
...
elseif valsels{2}
    ...
    INDS = find(SELTEMPMAP2);
    string0s = ['当前层: 矢状位 ',num2str(XDIMS),'(范围: 1-',num2str(dims(1)),')'];
    if length(INDS)>0
        string1s = ['层内ROI体素数目: ',num2str(nnz(SELTEMPMAP2))];
        almat = hdr.mat(1:3,1:3);
        vsize = [max(abs(almat(1,:))),max(abs(almat(2,:))),max(abs(almat(3,:)))];
...
%     end
    ...
    INDS = find(SELTEMPMAP2);
    string0s = ['当前层: 冠状位 ',num2str(YDIMS),'(范围: 1-',num2str(dims(2)),')'];
    if length(INDS)>0
        string1s = ['层内ROI体素数目: ',num2str(nnz(SELTEMPMAP2))];
        almat = hdr.mat(1:3,1:3);
        vsize = [max(abs(almat(1,:))),max(abs(almat(2,:))),max(abs(almat(3,:)))];
...
%     volshow = round(vols);
%     string2s = ['层内ROI体积（毫升）: ',num2str(volshow/1000)];
%     string3s = ['层内ROI最小值: ',num2str(min(SELTEMPMAP(INDS)))];
%     string4s = ['层内ROI最大值: ',num2str(max(SELTEMPMAP(INDS)))];
...
for i = 1:length(ixsort)
    ...
    if length(datsort0)>4
        ...
        SagittalROIinfo{i+1,11} = kurtosis(TEMPDAT(INDEXSTEMP));
        SagittalROIinfo{i+1,12} = skewness(TEMPDAT(INDEXSTEMP));
    end
end
...
for i = 1:length(iysort)
    ...
    if length(datsort0)>4
        ...
        CornoralROIinfo{i+1,11} = kurtosis(TEMPDAT(INDEXSTEMP));
        CornoralROIinfo{i+1,12} = skewness(TEMPDAT(INDEXSTEMP));
    end
end
...
for i = 1:length(izsort)
    ...
    if length(datsort0)>4
        ...
        AxialROIinfo{i+1,11} = kurtosis(TEMPDAT(INDEXSTEMP));
        AxialROIinfo{i+1,12} = skewness(TEMPDAT(INDEXSTEMP));
    end
end
