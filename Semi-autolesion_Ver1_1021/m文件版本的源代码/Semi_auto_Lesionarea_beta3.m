function Semi_auto_Lesionarea_beta3(dat,hdr,outputs)
...
HSAL.total = figure('menubar','none',...
    ...
    'name','Semi-Auto Lesion Area Selection toolkit Version 1.0');
...
HSAL.Graysel_text = uicontrol('parent',HSAL.total,...
    ...
    'style','text',...
    'string',['Current val',['  (minv ',num2str(round(minv)),';maxv ',num2str(round(maxv)),')']]);
...
DATXtemp = str2num(DATX(10:end));
DATY= get(HSAL.Ypos,'string');
DATYtemp = str2num(DATY(10:end));
DATZ= get(HSAL.Zpos,'string');
DATZtemp = str2num(DATZ(10:end));
XDIMS = DATXtemp;
YDIMS = DATYtemp;
ZDIMS = DATZtemp;
OUTS = HSAL.OUT;
...
if ~isempty(dir([TEMPINFO.HSAL.OUT,Outnames]))
    butinfo = questdlg(['Exist saved files: ',Outnames],'Exist files','Restore','Cancel','Restore');
end
if strcmp(butinfo,'Cancel')
    Relatedinfo = load([TEMPINFO.HSAL.OUT,Outnames]);
    INDS = find(Relatedinfo.SEGMENTS);
    set(HSAL.RelatedInfo_text2,'string',{'Added ROI information',...
        ['Area voxelnum: ',num2str(nnz(Relatedinfo.SEGMENTS))],...
        Showinfo,['maxval: ',num2str(max(DAT0(INDS)))],...
        ['minval: ',num2str(min(DAT0(INDS)))],...
        ['meanval: ',num2str(mean(DAT0(INDS)))]})
    return;
end
...
INDS = find(SEGMENTS);
set(HSAL.RelatedInfo_text2,'string',{'ADDED ROI information',...
    ['Area voxelnum: ',num2str(nnz(SEGMENTS))],...
    Showinfo,['maxval: ',num2str(max(DAT0(INDS)))],...
    ['minval: ',num2str(min(DAT0(INDS)))],...
    ['meanval: ',num2str(mean(DAT0(INDS)))]})
...
if strcmp(butinfo,'Cancel')
    Relatedinfo = load([TEMPINFO.HSAL.OUT,Outnames]);
    INDS = find(Relatedinfo.SEGMENTS);
    set(HSAL.RelatedInfo_text2,'string',{['Area voxelnum: ',num2str(nnz(Relatedinfo.SEGMENTS))],...
        Showinfo,['maxval: ',num2str(max(DAT0(INDS)))],...
        ['minval: ',num2str(min(DAT0(INDS)))],...
        ['meanval: ',num2str(mean(DAT0(INDS)))]})
    return;
...
SEGMENTS = TEMPINFO.seg;
INDS = find(SEGMENTS);
set(HSAL.RelatedInfo_text2,'string',{['Area voxelnum: ',num2str(nnz(SEGMENTS))],...
    Showinfo,['maxval: ',num2str(max(DAT0(INDS)))],...
    ['minval: ',num2str(min(DAT0(INDS)))],...
    ['meanval: ',num2str(mean(DAT0(INDS)))]})
save([TEMPINFO.HSAL.OUT,Outnames],'MASKS','ITERNUMS','ALPHA','PHI','SEGMENTS');
...
set(HSAL.Graysel_text,'string',['Currentval max: ',num2str(valsmax0),',min: ',num2str(valsmin0)]);
...
set(HSAL.Xpos,'string',['L-R dim: ',num2str(round(dims(1)/2))])
set(HSAL.Ypos,'string',['P-A dim: ',num2str(round(dims(2)/2))])
set(HSAL.Zpos,'string',['I-S dim: ',num2str(round(dims(3)/2))])
...
if valsels{1}  %% val 1:dims(3)
    ...
    XDIMS = round(dims(1)/2);
    YDIMS = round(dims(2)/2);
    set(HSAL.Xpos,'string',['L-R dim: ',num2str(XDIMS)])
    set(HSAL.Ypos,'string',['P-A dim: ',num2str(YDIMS)])
    set(HSAL.Zpos,'string',['I-S dim: ',num2str(ZDIMS)])
    set(HSAL.Val,'string',['Current: ',num2str(HSAL.DAT(XDIMS,YDIMS,ZDIMS))]);
...
if valsels{1}
    ...
    set(HSAL.RelatedInfo_text2,'string',{['Area voxelnum: ',num2str(nnz(datmask))],...
        Showinfo,['maxval: ',num2str(max(DATSHOWRES(INDS)))],...
        ['minval: ',num2str(min(DATSHOWRES(INDS)))],...
        ['meanval: ',num2str(mean(DATSHOWRES(INDS)))]})
elseif valsels{2}
...
if valiter<=0
    errordlg('wrong iteration nums');
    return;
end
if valalpha>1||valalpha<0
    errordlg('wrong smooth parameter(0<alpha<1)');
    return;
end
...
SEGMENTS = seg;
INDS = find(SEGMENTS);
set(HSAL.RelatedInfo_text,'string',{['Area voxelnum: ',num2str(nnz(SEGMENTS))],...
    Showinfo,['maxval: ',num2str(max(DAT0(INDS)))],...
    ['minval: ',num2str(min(DAT0(INDS)))],...
    ['meanval: ',num2str(mean(DAT0(INDS)))]})
save('temp')
...
valsmax0 = get(HSAL.Graysel_slideMax,'val');
if valsels{1}
    answers = questdlg('Set range','Set range or Whole brain','Yes','No','Yes');
    SETRAGE = answers;
    if strcmp(SETRAGE,'Yes')
        answers = inputdlg({['low slice, larger than 0, Current: ',num2str(DATZtemp)],...
            ['high slice, smaller than ',num2str(dims(3)),', Current: ',num2str(DATZtemp)]},...
            'Slice Range',1,{'1',num2str(dims(3))});
        RANGE = round([str2num(answers{1}),str2num(answers{2})]);
        if RANGE(1)>=RANGE(2)
            error('Wrong Input: Low slice should be smaller than High slice');
        end
        if RANGE(1)<1
            error('Low slice must larger than 0');
        end
        if RANGE(2)>dims(3)
            error(['High slice must smaller than max dim (',num2str(dims(3)),')']);
        end
    else
        RANGE = [1,dims(3)];
    end
...
elseif valsels{2}
    ...
    SETRAGE = answers;
    if strcmp(SETRAGE,'Yes')
        answers = inputdlg({['low slice, larger than 0, Current: ',num2str(DATXtemp)],...
            ['high slice, smaller than ',num2str(dims(1)),', Current: ',num2str(DATXtemp)]},...
            'Slice Range',1,{'1',num2str(dims(1))});
        RANGE = round([str2num(answers{1}),str2num(answers{2})]);
        if RANGE(1)>=RANGE(2)
            error('Wrong Input: Low slice should be smaller than High slice');
        end
        ...
        if RANGE(2)>dims(1)
            error(['High slice must smaller than max dim (',num2str(dims(1)),')']);
        end
    else
        RANGE = [1,dims(1)];
    end
    ...
    SETRAGE = answers;
    if strcmp(SETRAGE,'Yes')
        answers = inputdlg({['low slice, larger than 0, Current: ',num2str(DATYtemp)],...
            ['high slice, smaller than ',num2str(dims(2)),', Current: ',num2str(DATYtemp)]},...
            'Slice Range',1,{'1',num2str(dims(2))});
        RANGE = round([str2num(answers{1}),str2num(answers{2})]);
        if RANGE(1)>=RANGE(2)
            error('Wrong Input: Low slice should be smaller than High slice');
        end
        ...
        if RANGE(2)>dims(2)
            error(['High slice must smaller than max dim (',num2str(dims(2)),')']);
        end
    else
        RANGE = [1,dims(2)];
    end
...
uiwait(msgbox('Write ROI successfully!'))
end
